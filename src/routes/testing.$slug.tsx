      ...
      <section className="bg-brand-deep text-white py-20">
        <div className="container-x">
          <nav className="flex items-center gap-1 text-xs text-white/60">
            ...
            <Link to="/testing" className="hover:text-amber">Testing</Link>
            <ChevronRight className="w-3 h-3" />
            <span className="text-white/90">{service.title}</span>
          </nav>
          <div className="mt-4 text-xs uppercase tracking-widest text-amber font-semibold">Testing Service</div>
          ...
          <Reveal className="lg:col-span-2">
            ...
            <div className="mt-10 space-y-10">
              ...
              {service.standards?.length ? (
                <div>
                  <h3 className="text-xl font-bold text-brand-deep font-display">Testing Standards</h3>
          ...
          <aside className="space-y-6">
            <div className="rounded-lg bg-surface border border-border p-6">
              ...
              <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
                <li className="flex items-start gap-2"><Check className="w-4 h-4 text-amber mt-0.5" /> ASTM · ISO · EN · BIS</li>
                <li className="flex items-start gap-2"><Check className="w-4 h-4 text-amber mt-0.5" /> Customer-defined protocols</li>
                <li className="flex items-start gap-2"><Check className="w-4 h-4 text-amber mt-0.5" /> Traceable calibration</li>
              </ul>
            </div>
            <div className="rounded-lg bg-brand text-white p-6">
              <h3 className="font-bold">Discuss your specimen</h3>
              <p className="text-sm text-white/70 mt-2">Send us your standard, geometry and required test conditions — we'll come back with a plan.</p>
      ...
      <section className="py-16 bg-surface">
        <div className="container-x">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {testingServices.filter((t) => t.slug !== service.slug).slice(0, 4).map((t) => (
              <Link key={t.slug} to="/testing/$slug" params={{ slug: t.slug }} className="rounded-lg border border-border bg-white p-5 hover:shadow-md hover:-translate-y-1 transition">
                <div className="text-xs uppercase tracking-widest text-amber font-bold">Testing</div>
